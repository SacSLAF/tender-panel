            ...
            <nav class="mt-4">
                <div class="px-4 space-y-1">
                    <a href="{{ route('admin.dashboard') }}"
                        class="flex items-center px-4 py-2 rounded-lg {{ request()->routeIs('admin.dashboard') ? 'bg-blue-100 text-blue-600' : 'text-gray-600 hover:bg-gray-100' }}">
                    ...
                    <a href="{{route('admin.index')}}" class="flex items-center px-4 py-2 rounded-lg {{ request()->routeIs('admin.index') ? 'bg-blue-100 text-blue-600' : 'text-gray-600 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.tenders.index') }}"
                        class="flex items-center px-4 py-2 rounded-lg {{ request()->routeIs('admin.tenders.*') ? 'bg-blue-100 text-blue-600' : 'text-gray-600 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @if (session('success') || session('error') || session('warning'))
        <script>
            Swal.fire({
                ...
                title: '{{ session('error') ? 'Error' : (session('warning') ? 'Warning' : 'Success') }}',
                text: '{{ session('error') ?? (session('warning') ?? session('success')) }}',
                timer: 3000,
